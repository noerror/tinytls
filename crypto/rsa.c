...
#ifdef EAP_TLS_FUNCS
static const unsigned char * crypto_rsa_parse_integer(const unsigned char *pos, const unsigned char *end,
                                           struct bignum *num)
{
        struct asn1_hdr hdr;
        ...
        if (asn1_get_next(pos, end - pos, &hdr) < 0 ||
			hdr.classtype != ASN1_CLASS_UNIVERSAL || hdr.tag != ASN1_TAG_INTEGER) {
                /*wpa_printf(MSG_DEBUG, "RSA: Expected INTEGER - found class %d "
                           "tag 0x%x", hdr.classtype, hdr.tag);*/
                return NULL;
        }
...
struct crypto_rsa_key *
crypto_rsa_import_public_key(const unsigned char *buf, unsigned int len)
{
        struct crypto_rsa_key *key;
        struct asn1_hdr hdr;
        const unsigned char *pos, *end;

		key = (struct crypto_rsa_key*)malloc(sizeof(*key));
        if (key == NULL)
                return NULL;
...
struct crypto_rsa_key *
crypto_rsa_import_private_key(const unsigned char *buf, unsigned int len)
{
        struct crypto_rsa_key *key;
        struct bignum *zero;
        struct asn1_hdr hdr;
...
int crypto_rsa_exptmod(const unsigned char *in, unsigned int inlen, unsigned char *out, unsigned int *outlen,
