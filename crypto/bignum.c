...
{
	struct bignum *n = (struct bignum*)malloc(sizeof(mp_int));
	if (n == NULL)
		return NULL;
...
*/
unsigned int bignum_get_unsigned_bin_len(struct bignum *n)
{
...
*/
int bignum_get_unsigned_bin(const struct bignum *n, unsigned char *buf, unsigned int *len)
{
	unsigned int need = mp_unsigned_bin_size((mp_int *)n);
	if (len && need > *len) {
		*len = need;
		return -1;
	}
...
* Returns: 0 on success, -1 on failure
*/
int bignum_set_unsigned_bin(struct bignum *n, const unsigned char *buf, unsigned int len)
{
	if (mp_read_unsigned_bin((mp_int *)n, (unsigned char *)buf, len) != MP_OKAY) {
		/*wpa_printf(MSG_DEBUG, "BIGNUM: %s failed", __func__);*/
		return -1;
	}
	return 0;
